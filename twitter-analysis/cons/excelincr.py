        ...
        for line in dataFile:
            ...
            if int(id_str) not in c.keys():
                ...
                if 'created_at' in data['user']:
                    ur_dt = data["user"]["created_at"]
    	            ur_dt  = ur_dt.replace("+0000 ", "")

    	        print("%s$%s$%s$%s$%s$%s$%s$%s$%s$%s$%s$%r$%r$%r$%s$%s" %(id_str, tweet_type, cr_dt, in_altmetric, user, st_cnt, fc_cnt, fr_cnt, rt_cnt, rt_from, reply_to, hashes, text, rt_status, reply_to_status, ur_dt))
