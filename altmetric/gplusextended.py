...
# Usage: gplusextended.py id_of_publication extended > altmetricextended.csv
#
# This dumps the extended output of Google+ posts available on altmetrics
# entry = gplusId + '$' + gpostId + '$' + dateP + '$' + timeP + '$' + summary + '$' + homepage
...
class GPlus(AltmetricBase):
    ...
    @staticmethod
    def checkUsage(*argv):
        USAGE = "gplus.py altmetricId postids|user"
        if len(argv) != 3:
    ...
    def findPosts(self, soup):
        tHandle = []
        postTag = soup.find_all("article", {"class" : "post gplus"})
        # find the Google Post Id if available from href
        for post in postTag:
           gplusId = "+_unknown"
           gpostId = ""
           dateP = ""
           ...
           for a in aTag:
               ...
               if gplusId  == "+_unknown":
                   ...
                   try:
                       ...
                       self.logger.debug("%r", e.args)
                       self.logger.debug("Original Tag %r", a)
               timeTags = a.find_all('time')
               for timeT in timeTags:
                     part = timeT.get('datetime')
                     if part:
                         dateP = part.split('T')[0]
                         timeP = part.split('T')[1]
               summaryTags = a.find_all('p', {'class':'summary'})
               for st in summaryTags:
                   summary = st.text
               y =  a.get('href').split(u'/')[3]
               if y:
                   homepage = 'https://plus.google.com/' + y
               else:
                   homepage = ''
               entry = gplusId + '$' + gpostId + '$' + dateP + '$' + timeP + '$' + summary + '$' + homepage
               tHandle.append(entry)
        return tHandle

    def executePosts(self):
        posts = []
        status, soup = self.openAndLoadURL(self.amUrl)
        if status:
            posts = self.findPosts(soup)
            self.logger.debug('Found %d posts so far', len(posts))

            for soup in self.getMoreSoup():
                posts.extend(self.findPosts(soup))
                self.logger.debug('Found %d posts so far', len(posts))

            self.logger.info('Found %d posts in total for the link', len(posts))
            posts = list(set(posts))
            self.logger.info('Found %d Unique Posters for the link', len(posts))
            for post in posts:
                print (post).encode('utf-8')
            self.logger.info('written all the posts to stdout...')
        else:
